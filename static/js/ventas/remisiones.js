...
function buscarCliente () {
  ...
  .done(function (data) {
    ...
    for(let j = 0, lon = classResult.children.length; j < lon; j++){
      let click = classResult.children[j]
      click.onclick = function(){
        let cc = this.getAttribute('data-cc')
...
function buscarProductos () {
  let valuePro = document.getElementById('pro_search').value
  let selectPre = document.getElementById('selectprecios').value
  $.ajax({
    url: 'producto/',
    type: 'POST',
    data: {data: valuePro, precio: selectPre}
  })
  .done(function(data){
    var idClass = document.getElementById('resultPro')
    var html = ''
...
function FacturaPdf (dat, nume) {
  ...
  for (var i = 0; i < dat.length; i++) {
    rows.push([dat[i].codigo, dat[i].nombre, dat[i].valor, dat[i].cantidad, dat[i].iva,
    dat[i].valneto])
