...
function saveRemision() {
  ...
  for (let i = 0; i < tbody.children.length; i++) {
    ...
    allSAve.res.push({ codigo: codPro,
      cantidad: canPro,
      valorU: valPro,
      iva: ivaPro,
      valorN: valNeto });
...
function FacturaPdf(dat) {
  ...
  const columns = ['CODIGO', 'PRODUCTO', 'VALOR U', 'CANTIDAD', 'IVA', 'TOTAL'];
  const rows = [];
  let tot = 0;
  let iva = 0;
  const desc = 0;
  ...
  doc.autoTable(columns, rows, { addPageContent(data) {
...
function moneda(number) {
  const dato = new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'USD' }).format(number);
  return dato;
}
