...
function select(ele) {
  ...
  if (item.length === 0) {
    items.venta.push({id: ele.dataset.key, name: ele.dataset.name, iva: ele.dataset.iva, precio: ele.dataset.precio, cantidad: 1})
    reRender(items.venta)
  } else {
...
function buscarCliente() {
  ...
  if (this.value.length === 0) {
    ...
    fetch('clientepos/', myInit).then((response) => {
      return response.json()
    }).then((data) => {
...
function saveVenta() {
  ...
  fetch('save/', myInit).then((response) => {
    return response.json()
  }).then((data) => {
    items.cliente = {}
    items.venta = []
