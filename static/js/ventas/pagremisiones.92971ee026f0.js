...
function simpleTemplating(data) {
  ...
  $.each(data, function(index, item) {
    let estado = item.estado === 'cerrada'
      ? `<td class="positive">${item.estado}</td>`
      : `<td class="negative">${item.estado}</td>`
    let data = moneda(item.total)
    html += `<tr><td>${item.id}</td><td>${item.fecha}</td>
              <td>${item.identificacion}</td>
              <td>${item.nombre}</td><td >${data}</td>
              ${estado}
              <td><a href="detalle/${item.id}"  data-tooltip="Ver" ><i  class="large circular link teal unhide icon"></i></a>
