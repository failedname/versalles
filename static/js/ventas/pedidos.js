document.addEventListener('DOMContentLoaded',function(){
    ...
    let inputCliente = document.getElementById('cliente_search')
    inputCliente.addEventListener('input', (e) => {
        ...
        if (valInput.length > 0) {
            ...
            $.ajax({url: 'cliente/', type: 'POST', data: valInput}).done(function(data) {
        ...
        var btnAgregar = document.getElementById('btnAgregar')
          ...
          function calcular(tagbody) {
            ...
            var abono =  document.getElementById('id_abono')
            ...
            abono.value = 40 * tot / 100

        
          } 
          
          
          var btnsaveReal = document.getElementById('guardarpedido')
            ...
            function saveFactReal(e) {
              ...
              let valorAbono = document.getElementById('id_abono').value
              allSAve.abono = {
                'valor': valorAbono
              }
              allSAve.cliente = {
              ...
              $.ajax({url: 'guardar/', data: data, type: 'POST'}).done(function(res) {
                removeAll()
          
                FacturaPdf(res.data,res.abn)
            ...
            function FacturaPdf(dat, abn) {
              ...
              doc.setFontSize(13)
              var cod = dat[0].pedido
              doc.text(155, 20, 'PEDIDO No.')
              ...
              doc.setFontSize(9)
              doc.setTextColor(0)
              doc.text(162, 64, 'Fecha de Pedido')
              
              var fecha = dat[0].fecha
              doc.text(165, 69, fecha.toString())
              doc.text(165, 75, 'Año  Mes  Dia')
              
              doc.rect(155, 77, 50, 17, 'D')
              var nombre = dat[0].cliente
              var nit = dat[0].nit
              var tel = dat[0].telefono
              var dire = dat[0].direccion
              doc.setFontSize(10)
              doc.setFontSize(10)
              doc.setTextColor(100)
              ...
              doc.autoTable(columns, rows, {
                addPageContent: function(data) {
                  ...
                  doc.setTextColor(0)
                  doc.text(data.settings.margin.left + 127, doc.internal.pageSize.height - 55, 'VALOR PEDIDO:')
                  doc.text(data.settings.margin.left + 127, doc.internal.pageSize.height - 48, 'VALOR ABONOS:')
                  doc.text(data.settings.margin.left + 127, doc.internal.pageSize.height - 41, 'SALDO:')
                  doc.setFontSize(10)
                  var TotFormat = moneda(tot)
                  let BrutoFormat = moneda(bruto)
                  let IvaFormat = moneda(iva)
                  let DescFormat = moneda(desc)
                  let abonoMoneda = moneda(abonos)
                  let saldo = moneda(tot - abonos)
                  doc.text(data.settings.margin.left + 167, doc.internal.pageSize.height - 55, TotFormat.toString())
                  doc.text(data.settings.margin.left + 167, doc.internal.pageSize.height - 48, abonoMoneda.toString())
                  doc.text(data.settings.margin.left + 167, doc.internal.pageSize.height - 41, saldo.toString())
                  ...
                  doc.setFontSize(7)
                  var vivero = dat[0].vivero.toUpperCase()
