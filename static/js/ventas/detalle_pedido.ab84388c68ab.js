new Vue({
    ...
    methods:{
      ...
      copiaPedido(){
        ...
        fetch('copia_pedido/',myInit)
            .then((res)=>{
                return res.json()
            })
            .then((data)=>{
                console.log(data)
                pedidoPdf(data.data,data.abn)
            })
      }  
    }
...
function generarRecibo(datos){
    ...
    doc.setFontSize(13)
    var cod = dat[0].abono
    doc.text(150, 25, 'RECIBO DE CAJA No.')
    doc.text(165, 35, cod.toString())
    doc.setFillColor(150)
    doc.roundedRect(5, 45, 200, 10, 1, 1, 'F')
    doc.setTextColor(0)
    ...
    doc.autoTable(columns, rows, {
      addPageContent: function (data) {
        ...
        doc.setTextColor(0)
        doc.text(data.settings.margin.left + 127, doc.internal.pageSize.height - 34, 'Total:')
        doc.setFontSize(10)
        var TotFormat = moneda(tot)
        let BrutoFormat = moneda(bruto)
        let IvaFormat = moneda(iva)
