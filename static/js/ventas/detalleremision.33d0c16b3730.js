document.addEventListener('DOMContentLoaded', () => {
  ...
  function detalleFac() {
    ...
    let print = detalle[0].estado === 'cerrada'
      ? `<a  id="print" dataa-id="${detalle[0].remision}" data-tooltip="Imprimir Copia" ><i  class="large circular link teal print icon"></i></a>`
      : null
    ...
    let html = `<p>Factura: ${detalle[0].remision}</p>
                <p>Cliente: ${detalle[0].cliente}</p>
                <p>Fecha: ${detalle[0].fecha}</p>
                <p>NIT/CC: ${detalle[0].nit}</p>
    ...
    for (var i = 0; i < tamaño; i++) {
      htmlbody += `<tr>
                  ...
                  <td>${moneda(detalle[i].valneto)}</td>
                </tr>
                `
    }
