  ...
  methods: {
    saveInvoice() {
      ...
      if (Object.keys(this.info.cliente).length && this.info.datos.length) {
        ...
        fetch('savefactura/', myInit).then((res) => {
          return res.json()
        }).then((data) => {
          this.info.cliente = {}
          this.info.datos = []
          this.info.pago = ''
          FacturaPdf(data.data, data.nume)
    ...
    buscarProducto() {
      if (this.producto.length > 3) {
        ...
        var myInit = {
          method: 'POST',
          body: JSON.stringify({valinput: this.producto, precio: this.precio.precio}),
          headers: myHeaders,
          credentials: 'include'
        }
